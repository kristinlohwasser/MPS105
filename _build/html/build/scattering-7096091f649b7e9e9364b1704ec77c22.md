...
Therefore, let's take a look at the left-hand side of the equation. Again, ${v'^2_t}$ will always be positive, so $\left(1 - \frac{m_t}{m_\alpha}\right)$ needs to be negative:

$$\left(1 - \frac{m_t}{m_\alpha}\right)<0$$
$$
\begin{align*}
\left(1 - \frac{m_t}{m_\alpha}\right)&<0 \\
1 &< \frac{m_t}{m_\alpha} \\
...
This shows that the target mass is larger than that of the $\alpha$ particle. In fact it's much larger by roughly a factor of 40: M(Au) $\sim$ 40 $\times$ M(alpha). This indicates, there is a solid heavy "ball" inside the atom, not a soft distributed mass (with a weight much smaller than the )
