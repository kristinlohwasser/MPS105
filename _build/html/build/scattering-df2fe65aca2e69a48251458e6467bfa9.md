...
The differential cross-section is determined as the particles flux scattered into the solid angle $\Delta \Omega$ accepted by the detector:

$$frac{\Delta J}{J A} = n_b \Delta \times frac{ d \sigma}{d \Omeage} \Delta \Omega$$

Differential cross-section:
Defined as the fraction of the particle flux that is scattered into the solid angle ΔΩ accepted by the detector.





Determined by particle density nb of scatterers B, the length Δx which the incident beam of particles A traverse through the scattering volume V and the differential scattering cross section dσ/dΩ which depends on the interaction potential between particles A and B.
...
Elastic collision: Kinetic energy is conserved (for example snooker balls)
The final momenta depend on the angle of scattering. e.g. given θ, we can calculate v1 and v2

Inelastic collision: Kinetic energy is not conserved (for example car crash)
Kinetic energy is transformed into heat or other mechanical energy

In a scattering process momentum is conserved but this is not always the case for the kinetic energy
