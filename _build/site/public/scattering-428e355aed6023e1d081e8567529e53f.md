...
From geometrical considerations (i.e. just defining the rate of particles scattered by an angle $\theta$ into a solid angle) we know that:

\begin{align*}
\frac{\mathrm{d} J (\theta, \textrm{d}\Omega)}{J A} = \frac{n_G}{A} V \frac{\mathrm{d}\sigma}{\mathrm{d}\Omega} = n_G \Delta x \frac{\textrm{d}}{\textrm{d} \theta} \textrm{d} \theta \textrm{d} \phi 
\end{align*}
