...
The results showed that most alpha particles passed straight through the foil however some were deflected at large angles of more than 90 degrees and even up to 180 degrees.

plum pudding model of the atom and led to the conclusion that atoms have a tiny, dense, positively charged nucleus

The physical phenomenon was explained by Rutherford in a classic 1911 paper{cite}`Rutherford:1911zz`
...
Velocities are always positive (>zero)
A scattering angle > 90 deg => cosΘ < 0 (negative), therefore: (1- mt / mα) < 0 (also negative)
The target mass is much larger than the alpha particle
A solid heavy “ball” inside the atom, not a soft distributed mass
M(Au) ~ 40 × M(alpha)
e.g. ping pong ball (M=3 g) hitting a Cricket ball (M=150 g)
