\section{Part 1: Investigating the microscopic World}

\subsection{Resolving small structures}
...
\begin{framed}
\textbf{Equation}\\
\textbf{The de Broglie wavelength}
...
\begin{framed}
\textbf{Example}\\
\textbf{Example: Wavelength of a proton travelling with v=0.01c}
...
\subsection{The Geiger-Marsden experiment}
...
\begin{framed}
\textbf{Calculation}\\
\textbf{Scattering angle in the Geiger Marsden Experiment}
...
\includegraphics[width=3\linewidth]{files/rutherfordscattering-984d9e190f37af67b350a60b287d121b.png}

Energy conservation:   $\,\,\,\,\,\,\,\,\,\, \frac{1}{2} m_\alpha {v^2_\alpha} = \frac{1}{2} m_\alpha {v'^2_\alpha} + \frac{1}{2} m_t {v'^2_t}$ \newline

Momentum conservation: $\,\,\, m_\alpha \vec{v_\alpha} = m_\alpha \vec{v'_\alpha} + m_t \vec{v'_t}$ \newline
...
Looking at the right-hand side, we find we have the multiplication of two velocities - this product is for sure positive (as there are not negative velocities). The other term on the right-hand side is $\cos\theta$ which is positive between -90° and +90°. However, $\cos\theta$ is negative for any angles larger than 90°. So the right-hand side takes on a {\textbackslash}textit\{negative\} value for scattering angles larger than 90°. This of course means, that also the left-hand side needs to be negative in these cases.
...
\begin{framed}
\textbf{Calculation}\\
\textbf{Distance of closest approach in the Geiger Marsden Experiment}
...
(see also PHY11006 lecture notes: {\textbackslash}textit\{5.5 The Potential and Potential Energy of a System of Charges\})
...
\subsection{General Basics of scattering experiments in High energy physics}
...
\begin{framed}
\textbf{Equation}\\
\textbf{Relativistic relationship between energy, momentum and mass}
...
\begin{framed}
\textbf{Example}\\
\textbf{Example: Mass of the Proton}
...
\bigskip\subsection{Cross-sections and their measurement}

\subsubsection{What is a cross-section?}

In the Geiger-Marsden experiment, whilst some $\alpha$ particles are scattered by large angle, others are not deflected at all. As we have seen, from the general considerations, we can determine an upper limit on the size~of~the~gold~atoms.
...
\subsubsection{Measurement of cross-sections}
...
\includegraphics[width=5\linewidth]{files/inclcrosssection-51bb14d4b2670cee975e54aa092a334c.png}
...
\subsection{The interpretation of the Geiger-Marsden experiment by Rutherford}
...
\begin{itemize}
\item express the fraction of the flux passing through the band/annular $b$ and deflected by an angle $\theta$


\item compare this to the general expression for the differential cross section


\item connect it to the expression of the impact parameter derived using angular momentum and the potential (\cite{Y1Mech})
\end{itemize}

\includegraphics[width=5\linewidth]{files/rutherfordscattering-6ebee0d845ccfa46d7b8e2208da3d98d.png}
...
\begin{framed}
\textbf{Calculation}\\
\textbf{Rutherford scattering angle}
...
The fraction of \textit{all} incident particles $\alpha$, scattered by \textit{all} atoms G with density $n_G$ in the volume $V$ = $A \Delta x$ is then:

\begin{align*}
\frac{\mathrm{d} J (\theta, \textrm{d}\Omega)}{J} &= n_G A \Delta x b \frac{\textrm{d}b}{\textrm{d} \theta} \textrm{d} \theta \textrm{d} \phi 
\end{align*}

This is a specific cross-section (in terms of geometry) that takes into account the impact paramter $b$ and the scattering angle $\theta$. We can compared it to  (\ref{my_label}) and find:
...
\begin{framed}
Measuring the relative fraction $\frac{\Delta J}{J}$ yields the differential cross section (see (\ref{my_label})) and with it the interaction potential as we have seen that the angle $\theta$ is defined as (see eq. (72) of the mechanics lecture notes):
...
As $b$ cannot be measured, we eliminated it from from (\ref{my_label2}) by inserting the above expression for $b$ and use $\sin(2\theta) = 2 \sin(\theta) \cos(\theta)$:
\begin{align*}
...
\textbf{This is the formula for Rutherford scattering!!}
\end{framed}
...
\subsection{Scattering on nucleons}
...
\includegraphics[width=5\linewidth]{files/falloffRutherfordFor-e6984e570b7ea4a5756f9c6196cc9bf0.png}
...
In the following decades, with the advent of \href{/accelerators}{particle accelerators}, the particle beams used in scattering experiments became more powerful. This triggered an era of scattering experiments with nucleons and an effort to understand the nucleus.
...
\includegraphics[width=5\linewidth]{files/ealastc-b32003eebf309d7bf5953a84f3cbe7d2.png}

Energy spectrum of 15.7 Mev electrons scattered at 30° by polystyrene (carbon). The ordinate (y-axis) represents the observed count as a function of an arbitrary energy unit on the x-axis (i.e. in this case the setting on the magnetic analyzer that selects out the energy of the scattered electrons). The
...
\includegraphics[width=3\linewidth]{files/inelastic-455a3cc3c9ceb91400521bafee2df248.png}

The elastic and inelastic peaks at 187 Mev in carbon at a scattering angle of 80°. (taken from \cite{PhysRev.99.1503})

\subsection{Deep inelastic scattering and the discovery of proton substructure}
...
Experiments conducted in 1968 at the Stanford Linear Accelerator Center (SLAC) and published on October 20, 1969, showed that the proton contained much smaller, point-like objects and was therefore not an elementary particle. The results no longer matching what was expected for elastic collisions on nucleons and were instead consistent with proton sub-structure.  Quarks were confirmed as constituents of the proton.

\includegraphics[width=4.5\linewidth]{files/halzen-b0532b9614326bf5fed4682ac1a9b620.png}
